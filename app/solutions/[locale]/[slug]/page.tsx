...
export async function generateMetadata({
  ...
  const languages = LOCALES.reduce<Record<string, string>>((acc, l) => {
    ...
    if (slugForLocale) {
      acc[l] = `${SITE_URL}/solutions/${l}/${slugForLocale}`;
    }
    return acc;
  }, {});
  if (slugsByLocale.en) {
    languages["x-default"] = `${SITE_URL}/solutions/en/${slugsByLocale.en}`;
  }

  const url = `${SITE_URL}/solutions/${locale}/${slug}`;
...
export default async function SolutionDetail({
  ...
  const switcherAlternates = LOCALES.reduce<Partial<Record<Locale, string>>>(
    (acc, l) => {
      const slugForLocale = slugsByLocale[l];
      if (slugForLocale) {
        acc[l] = `/solutions/${l}/${slugForLocale}`;
      }
      return acc;
    },
    {},
